...
This one is a failure, and I'll tell it as one. Not a technical failure — the fix was trivial and I designed it twice. It failed because waiting for a vendor to solve a problem someday felt safer than solving it in-house this year — and because I never made the cost of waiting visible enough to change that. Certificates kept expiring in production. Every time, they were renewed after they had already failed, and every time by a different person.

The setup, if you want to reproduce the failure yourself: two different teams bought certificates, from two different suppliers. Neither team monitored them. One supplier does at least email you when a certificate is due for renewal — except the email went to a mailbox nobody watched. The other doesn't tell you at all. So the only signal that a certificate needed renewing was the certificate failing. The service goes down, someone scrambles, someone renews it, and because there's no owner and no record, next time it's a different someone scrambling. There is no learning curve when nobody's on the same curve twice.
...
I was told it was too big a job, and that I'd need someone from the project office to help me structure it. Fine — a colleague from the project office did help, and did it well. I drew the whole thing as one picture: a classic workflow, who does which step, where the service desk sits, how an item enters the inventory. It wasn't complicated. My colleague agreed with it, tidied it, and presented it to senior management.

And that's where it stopped. The plan was easy and it went nowhere, because the preference was to wait for a future platform to manage certificates for us. There's a pattern in some organisations where any internal improvement can be deferred indefinitely by pointing at a future vendor capability: don't build the inventory, because one day a platform will make inventories unnecessary. Don't fix the process, because a someday-migration will make the process moot. The someday never arrives on a schedule you can plan around, and in the meantime the certificates keep expiring in production, renewed only after they fail, each time by whoever happens to be nearest.
...
- [ACME — automated certificate management (RFC 8555)](https://www.rfc-editor.org/rfc/rfc8555)
