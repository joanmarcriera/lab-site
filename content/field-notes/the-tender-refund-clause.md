...
It was the first time we'd put an acceptance test into one of our storage tenders. It paid for itself almost immediately — during acceptance, the system returned a cheerful S3 "200 OK" for a full petabyte of writes, and only 700 terabytes were actually readable. Silent data loss, caught by contract instead of by disaster. The vendor, to their credit, was deeply invested and fixed it fast. But without that clause, we'd have found out the day we needed the data.

What the tender replaced had been a reasonable design when it was built; it just hadn't been re-examined since. The disaster-recovery copy lived on enterprise NAS at a cost per petabyte that belonged to a different decade. The tape layer behind it: files bundled into full-tape tar archives via a parallel-filesystem staging area, orchestrated by two relational databases, written to tape — and the restore path had never been written down. Restoring one file meant reading and unpacking an entire tape. Tapes only get bigger; a design that must read a whole tape to return one file gets worse every year on its own. That's what an unexamined design looks like in infrastructure: not a bad day, a bad slope.
