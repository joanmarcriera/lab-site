...
I watched a version of this at the institute, where I ran data and infrastructure services for years. For a long time monitoring and metrics were an enthusiast's pursuit rather than a shared practice, and teams — mine included — were cautious about sharing numbers across boundaries. My reading, then and now: teams that withhold their metrics are rarely being shy. Opacity is a rational defence wherever numbers have ever been used against you, and the organisation, including me, had not yet made the separation between steering and judging explicit. The fix is not more dashboards. The fix is separating the numbers used to steer and learn from the numbers used to judge. SRE culture reached the same conclusion by a different road: [blameless postmortems](https://sre.google/sre-book/postmortem-culture/) exist precisely so that the learning channel is not poisoned by the evaluation channel.
